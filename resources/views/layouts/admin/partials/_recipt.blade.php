<div class="row justify-content-center" id="printableArea">
    <div class="col-5" id="printableAreaContent">
                ...
                @if (isset($order->customer))
                    <h5>
                        {{ translate('Customer Name : ') }}<span
                            class="font-weight-bold">{{ $order->customer['f_name'] . ' ' . $order->customer['l_name'] }}</span>
                    </h5>
                    <h5>
                        {{ translate('Phone : ') }}<span
                            class="font-weight-bold">{{ $order->customer['phone'] }}</span>
                    </h5>
                    <h5>
                        {{ ('Order Type : ') }}<span
                            class="font-weight-bold">{{ $order['order_type'] == 'dine_in' ? 'Table Order':$order['order_type']}}</span>
                    </h5>
                    @php($address = \App\Model\CustomerAddress::find($order['delivery_address_id']))
                    <h5>
                        {{ translate('Address : ') }}<span
                            class="font-weight-bold">{{ isset($address) ? $address['address'] : '' }}</span>
                    </h5>
                    @if ($order['delivery_date']!=null)
                    <h5>
                        Delivey : <span
                            class="font-weight-bold">{{ date('d/M/Y', strtotime($order['delivery_date'])) }} {{ $order['delivery_time']}}</span>
                    </h5>
                    @endif
                @endif
            </div>
        ...
        <h5>
            {{ ('Note: ') }}<span
                class="font-weight-bold">{{ $order['order_note'] }}</span>
        </h5>
